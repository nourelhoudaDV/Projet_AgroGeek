...
@section('breadcrumb')
    <x-group.bread-crumb page-tittle="{{ trans('pages/parcelle.edit_page_title') }}" :indexes="[
...
@section('content')
    @bind($model)
        <x-form.form method="post" action="{{ route('parcelles.update', $model[$model::PK]) }}">
                <x-form.card col="col-12 row" title='pages/parcelle.identification_parcelle'>
                    ...
                    <x-form.select col="col-12 col-sm-6" name="Ferme" label="Ferme" :bind-with="[\App\Models\Ferme::all(), ['idF', 'Ferme']]" />
                    ...
                    <x-form.select col="col-12 col-sm-6" name="typeSol" label="typeSol" :bind-with="[\App\Models\TypeSol::all(), ['idTS', 'typeSol']]" />
